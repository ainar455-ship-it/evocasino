...
export function GuideBonusRound({ rounds, className }: GuideBonusRoundProps) {
  return (
    <div className={cn("grid grid-cols-1 sm:grid-cols-2 gap-4 items-stretch", className)}>
      {rounds.map((round) => {
const Icon = round.icon || (() => null);
        return (
          <article
            key={round.name}
            className="border border-border/70 rounded-lg bg-card p-5 md:p-6 flex flex-col h-full transition-colors hover:border-border"
          >
            ...
            <div className="mt-auto pt-4 border-t border-border/60 grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-[13px] font-medium text-foreground font-body leading-tight tabular-nums">
                  {round.maxMultiplier}
                </p>
              </div>
            </div>
