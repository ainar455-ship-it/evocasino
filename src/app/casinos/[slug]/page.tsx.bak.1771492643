...
export default async function CasinoReviewPage({ params }: PageProps) {
  const { slug } = await params;
  const casino = CASINOS.find((c) => c.slug === slug);
  const casinoOffers = OFFERS.filter((o) => o.casino_id === casino?.id);
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <div>
              ...
              <div className="mt-2 space-y-1 text-xs">
                ...
                <div><a href={casino.out_url} target="_blank" rel="noreferrer" className="text-green-600 hover:text-green-700">{casino.name} Site</a></div>
              </div>
            </div>
            <div>
