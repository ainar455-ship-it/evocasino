...
function BonusRecommendationCard({
  ...
  return (
    <Link
      href={href}
      className="group relative flex cursor-pointer items-center gap-3 overflow-hidden rounded-lg border border-[hsl(var(--border)/0.6)] bg-[hsl(var(--card))] p-4 transition-colors duration-200 hover:border-[hsl(var(--gold)/0.3)] hover:bg-[hsl(var(--muted)/0.1)] md:gap-4 md:p-5"
    >
      <span
...
function getFeaturedBonusSummary(row: BonusRow, bonusScore: number) {
  ...
  return `Ranked from production bonus facts and Evolution scoring - Bonus Quality Score ${bonusScore}/100, Evolution Score ${casino.evolutionScore}.${verified}`;
}
