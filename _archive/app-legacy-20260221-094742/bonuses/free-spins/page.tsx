...
export default function FreeSpinsPage() {
  const { latestOffers } = getHomepageData();
  const freeSpinsOffers = latestOffers.filter((o) => o.type === "free_spins");
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">{offer.casinoName}</p>
                    </div>
                  ...
                  <div className="space-y-2 mb-4 text-sm flex-grow">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-gray-900">R{offer.minDepositZar}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Max Cashout:</span>
                      <span className="font-semibold text-gray-900">R{offer.maxCashoutZar}</span>
                    </div>
                    {offer.lastVerifiedAt && (
                      <div className="flex justify-between text-xs text-gray-500 mt-3 pt-3 border-t">
                        <span>Last verified:</span>
                        <span>{new Date(offer.lastVerifiedAt).toLocaleDateString("en-ZA")}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 mt-auto">
                    ...
                    <a href={`/casinos/`} className="rounded-lg border border-gray-300 bg-white px-4 py-3 text-center text-sm font-semibold text-gray-800 hover:border-gray-400 hover:shadow transition duration-200">Review</a>
                  </div>
                </div>
              );
            })}
