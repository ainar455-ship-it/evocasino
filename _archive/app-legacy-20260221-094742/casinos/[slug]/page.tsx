      ...
      <main className="min-h-screen bg-gray-50">
        <div className="mx-auto max-w-6xl px-4 py-10">
          <div className="rounded-lg border border-gray-200 bg-white p-12 text-center">
            ...
            <Link href="/" className="mt-4 inline-block rounded-lg bg-green-600 px-6 py-3 text-sm font-semibold text-white">
              Back to Home
            </Link>
          </div>
        </div>
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        <Link href="/" className="mb-4 inline-block text-sm text-green-600 hover:text-green-700 font-semibold">
          ← Back to all casinos
        </Link>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          {freeSpinsOffers.length > 0 && (
            <div className="rounded-lg border border-gray-200 bg-white p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-4">🎰 Free Spins Offers</h3>
                ...
                {freeSpinsOffers.map((offer) => (
                  <div key={offer.id} className="border-b pb-4 last:border-b-0">
                    ...
                    <div className="mt-3 space-y-1 text-sm text-gray-600">
                      ...
                      <p><span className="font-semibold">Min Deposit:</span> R{offer.minDepositZar}</p>
                      <p><span className="font-semibold">Max Cashout:</span> R{offer.maxCashoutZar}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {depositBonusOffers.length > 0 && (
            <div className="rounded-lg border border-gray-200 bg-white p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-4">💰 Deposit Bonuses</h3>
              <div className="space-y-4">
                {depositBonusOffers.map((offer) => (
                  <div key={offer.id} className="border-b pb-4 last:border-b-0">
                    <h4 className="font-semibold text-gray-900">{offer.title}</h4>
                    {typeof offer.bonusText === "string" && (
                      <p className="mt-2 text-2xl font-bold text-amber-600">{offer.bonusText}</p>
                    )}
                    <div className="mt-3 space-y-1 text-sm text-gray-600">
                      <p><span className="font-semibold">Wagering:</span> {offer.wagering}</p>
                      <p><span className="font-semibold">Min Deposit:</span> R{offer.minDepositZar}</p>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="rounded-lg border border-gray-200 bg-white p-6">
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              <li className="flex items-center gap-2"><span className="text-green-600 font-bold">✓</span> Licensed & Regulated</li>
              <li className="flex items-center gap-2"><span className="text-green-600 font-bold">✓</span> Mobile App Available</li>
              <li className="flex items-center gap-2"><span className="text-green-600 font-bold">✓</span> Fast Payouts (24-48 hours)</li>
              <li className="flex items-center gap-2"><span className="text-green-600 font-bold">✓</span> 24/7 Customer Support</li>
              <li className="flex items-center gap-2"><span className="text-green-600 font-bold">✓</span> Responsible Gambling Tools</li>
            </ul>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <div>
              ...
              <div className="mt-2 space-y-1 text-xs">
                <div><Link href="/" className="text-green-600 hover:text-green-700">All Casinos</Link></div>
                <div><Link href="/bonuses/free-spins" className="text-green-600 hover:text-green-700">Free Spins</Link></div>
